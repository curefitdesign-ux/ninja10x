...
const BottomNavBar = ({ hidden = false }: { hidden?: boolean }) => {
  ...
  const hiddenPaths = ["/auth", "/profile-setup", "/avatar-crop", "/camera", "/preview", "/gallery", "/progress", "/reel-generation"];
  ...
  const nav = (
    <div
      ...
      style={{
        bottom: "calc(max(env(safe-area-inset-bottom, 10px), 10px) + 4px)",
        zIndex: 40,
      }}
    >
