...
const FitnessFrame = ({ imageUrl, isVideo, activity, week, day, duration, pr, imagePosition, imageScale, label1, label2 }: FitnessFrameProps) => {
  ...
  return (
    <div className="w-full h-full rounded-[20px] overflow-hidden shadow-2xl relative" style={{ background: '#6B6B2A' }}>
      {/* Grid background */}
      ...
      <div className="absolute top-5 right-3 z-20 animate-subtle-pulse">
        <div className="px-2.5 py-1" style={{ background: '#F45B4A', borderRadius: '2px' }}>
          <span className="text-white font-bold text-[9px] tracking-[0.08em]" style={{ fontFamily: 'system-ui, sans-serif' }}>
            CONQUER WILL POWER
          </span>
        </div>
      </div>
      ...
      <div className="absolute top-12 left-4 z-20">
        <h1 
          ...
          style={{ color: '#F4E14D', fontFamily: 'system-ui, sans-serif', textShadow: '2px 3px 4px rgba(0,0,0,0.35)' }}
        >
          CULT
        </h1>
        <h2 
          className="text-[44px] font-black leading-[0.95] tracking-tight -mt-1"
          style={{ color: '#FFFFFF', fontFamily: 'system-ui, sans-serif', textShadow: '3px 4px 8px rgba(0,0,0,0.5)' }}
        >
          NINJA
        </h2>
        {/* Activity name badge */}
      ...
      <div 
        ...
        style={{ top: '22%', left: '50%', width: '58%', height: '44%', transform: 'translateX(-50%) rotate(-8deg)' }}
      >
        <div 
        ...
        <div className="absolute left-3 z-20 space-y-1.5" style={{ bottom: '26%' }}>
          {pr && (
            <div className="px-2.5 py-1 animate-subtle-float" style={{ background: '#F45B4A', borderRadius: '2px', animationDelay: '0.2s' }}>
              <span className="text-white font-bold text-[11px]" style={{ fontFamily: 'system-ui, sans-serif' }}>
                {metricLabel} : {pr}
              </span>
            </div>
          )}
