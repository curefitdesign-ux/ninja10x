...
export default function SendReactionSheet({ 
  ...
  // Build unified reaction list: activity-specific first, then core (deduplicated)
  const allReactions = useMemo(() => {
    const activityConfig = activityType ? ACTIVITY_REACTIONS[activityType] : null;
    const activitySpecific = activityConfig?.reactions || [];
    const seen = new Set<ReactionType>(activitySpecific);
    const coreFiltered = CORE_REACTIONS.filter(r => !seen.has(r));
    return [...activitySpecific, ...coreFiltered];
  }, [activityType]);
  ...
  return (
    <>
      ...
      >
          ...
          >
            ...
            <div className="flex items-center gap-4 overflow-x-auto overflow-y-visible scrollbar-none pb-2 pt-2 -mt-1">
                    ...
                    <div className="relative" style={{ overflow: 'visible' }}>
                      ...
                      {isCurrentUser && (
                        <button
                          ...
                          style={{ top: -14, right: -14, width: 44, height: 44, zIndex: 10 }}
                        >
                          <div 
                            className="w-5 h-5 bg-white/25 backdrop-blur-md rounded-full flex items-center justify-center"
                            style={{ border: '1.5px solid rgba(255,255,255,0.4)', boxShadow: '0 2px 8px rgba(0,0,0,0.4)' }}
                          >
              ...
              {actualReactionCount > 8 && (
                <button onClick={onViewReactions} className="flex flex-col items-center gap-1 flex-shrink-0">
                  <div className="w-11 h-11 rounded-full bg-white/10 flex items-center justify-center text-white/70 text-sm font-medium"
        ...
        {/* Divider */}
        {actualReactionCount > 0 && <div className="mx-5 h-px bg-white/10 my-2" />}

        {/* UNIFIED REACTION GRID */}
