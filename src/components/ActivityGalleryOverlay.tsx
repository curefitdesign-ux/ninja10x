...
const ActivityGalleryOverlay = forwardRef<HTMLDivElement, ActivityGalleryOverlayProps>(function ActivityGalleryOverlay({
  ...
  const overlay = (
    <AnimatePresence>
      {isOpen && (
        <motion.div className="fixed inset-0" style={{ zIndex: 60 }} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}>
                              ...
                              <div key={`week-complete-${completedWeek}`} className="relative flex items-center" style={{ padding: '12px 16px 12px 58px', margin: '6px 0' }}>
                                ...
                                <div style={{ display: 'inline-flex', flexDirection: 'column', gap: 0 }}>
                                  ...
                                  <svg width="90" height="6" viewBox="0 0 90 6" fill="none" style={{ marginTop: -1, marginLeft: 2, opacity: 0.25 }}>
                                    <path d="M2 4c14-3 28 1 42-1s24 2 44-1" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
                                  </svg>
                                </div>
                              </div>
                            );
                ...
                {showShareOptions && (
                  <>
                    <motion.div
                      ...
                      style={{ zIndex: 65, background: 'rgba(0,0,0,0.4)' }}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                    ...
                    />
                    <motion.div
                      className="fixed left-0 right-0 z-[66]"
                      style={{ bottom: 0 }}
                      initial={{ y: '100%' }}
                      animate={{ y: 0 }}
                      exit={{ y: '100%' }}
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          borderBottom: 'none',
                          padding: '20px 20px max(env(safe-area-inset-bottom, 20px), 24px)',
                        }}
                      >
                        {/* Handle */}
                        ...
                        <p className="text-white/60 text-xs font-medium tracking-wider uppercase mb-4 px-1">Share to</p>
                        <div className="flex gap-3">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              const payload = buildSharePayload();
                              const shareText = `${payload.text}\n\n${payload.url}`;
                              // WhatsApp Status deep link
                              const whatsappStatusUrl = `whatsapp://send?text=${encodeURIComponent(shareText)}`;
                          ...
                          <button
                            ...
                            onClick={() => {
                              setShowShareOptions(false);
                              const payload = buildSharePayload();
                              // Try Instagram Stories deep link (works on mobile with Instagram installed)
                              const igStoriesUrl = `instagram-stories://share?source_application=ninja10x`;
                              window.location.href = igStoriesUrl;
                              // Fallback: copy link and notify user
                              setTimeout(() => {
                                navigator.clipboard.writeText(`${payload.text}\n\n${payload.url}`)
                                  .then(() => toast.success('Link copied! Paste it in your Instagram Story.'))
                                  .catch(() => toast.error('Unable to copy link'));
                              }, 1500);
                            }}
                          >
                            <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, #833AB4, #E1306C, #F77737)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          ...
                          >
                            ...
                            <span className="text-white/80 text-xs font-medium">More</span>
                          </button>
                        </div>
                      </div>
                    </motion.div>
