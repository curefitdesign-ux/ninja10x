...
const Reel = () => {
  ...
  const isWithin24Hours = currentActivity ? Date.now() - new Date(currentActivity.createdAt).getTime() < 24 * 60 * 60 * 1000 : false;
  ...
  if (!userGroups.length || !currentGroup || !currentActivity) {
    return <div className="fixed inset-0 bg-black flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={handleClose} className="px-4 py-2 rounded-full text-white/80 bg-white/10">
          Go Back
        </button>
      </div>;
  }
  ...
  const currentReactions = localReactions[currentActivity.id] || {
    total: 0,
    reactions: {
      ...DEFAULT_REACTIONS
    },
  ...
  const activeReactionTypes = Object.entries(currentReactions.reactions).filter(([, r]) => r.count > 0).map(([type]) => type as ReactionType);
  ...
  return <DynamicBlurBackground imageUrl={mediaUrl}>
      ...
      overflow: 'hidden'
        ...
        <StoryHint hasMultipleStories={currentGroup?.activities.length > 1} hasMultipleUsers={userGroups.length > 1} onNudge={triggerShake} />

        {/* FIXED HEADER ZONE - Single row: delete, avatars, close + user name below */}
        <div className="shrink-0 z-50 flex flex-col" style={{
        paddingTop: 'max(env(safe-area-inset-top, 12px), 12px)'
      }}>
          {/* Row 1: Delete + Avatars + Close - all in one row */}
          <div className="flex items-center justify-between px-3 shrink-0" style={{
          height: 56
        }}>
            {/* Left side - Delete button */}
            <div className="w-10 shrink-0">
              {canEdit && <button onClick={() => setShowDeleteConfirm(true)} className="p-2 transition-colors rounded-full text-white/80 hover:text-red-400 min-w-[40px] min-h-[40px] flex items-center justify-center active:scale-95">
            ...
            <div className="flex-1 flex justify-center">
              <div className="flex items-center gap-2">
                ...
                return <button key={group.userId} onClick={() => {
                  setCurrentUserIndex(idx);
                  setCurrentActivityIndex(0);
                }} className="relative active:scale-95 transition-transform" style={{
                  transform: isActive ? 'scale(1)' : 'scale(0.9)'
                    ...
                    transform: 'rotate(-90deg)'
                      ...
                      const progressLength = isCurrentSegment ? segmentLength * autoAdvanceProgress : segmentLength;
                      return <g key={segIdx}>
                              {/* Background track for current segment */}
                              {isCurrentSegment && <circle cx="50" cy="50" r={radius} fill="none" strokeWidth="6" stroke="rgba(255,255,255,0.15)" strokeDasharray={`${segmentLength} ${circumference}`} strokeDashoffset={-offset} strokeLinecap="round" />}
                              {/* Segment fill */}
                              <circle cx="50" cy="50" r={radius} fill="none" strokeWidth="6" stroke={isStoryLocked ? 'rgba(255,255,255,0.15)' : isSegmentUnviewed ? 'rgba(255,255,255,0.25)' : 'url(#storyGradient)'} strokeDasharray={`${isCurrentSegment ? progressLength : segmentLength} ${circumference}`} strokeDashoffset={-offset} strokeLinecap="round" style={{
                          filter: !isStoryLocked && (isSegmentViewed || isCurrentSegment) ? 'drop-shadow(0 0 4px rgba(236, 72, 153, 0.5))' : 'none',
                    ...
                    padding: 4
                        ...
                        <div className="w-full h-full rounded-full overflow-hidden">
                          <ProfileAvatar src={group.avatarUrl} name={group.displayName} size={avatarSize - 8} style={{
            ...
            <div className="w-10 shrink-0 flex justify-end">
              <button onClick={handleClose} className="text-white/80 hover:text-white transition-colors p-2 min-w-[40px] min-h-[40px] flex items-center justify-center">
          ...
          <div className="flex items-center justify-center gap-2 shrink-0 px-4 pb-2" style={{
          height: 28
        }}>
        ...
        <div className="flex-1 flex items-center justify-center z-30 overflow-hidden px-4 relative">
          {/* Card container with shake animation - single wrapper */}
          <motion.div className="relative w-full max-w-[340px] flex items-center justify-center bg-muted-foreground" style={{
          maxHeight: '100%'
        }} animate={shakeAnimation} transition={shakeTransition} onClick={handleTap}>
              {/* Full templated image/video - with lock overlay for non-public users */}
              {(() => {
            const shouldShowLocked = !isOwnStory && !hasPublicActivity;
            return <div className="relative w-full max-w-[340px] flex items-center justify-center">
                    {isVideo ? <video key={mediaUrl} src={mediaUrl} className="w-full h-auto rounded-2xl" style={{
                maxHeight: 'calc(100dvh - 240px)',
                objectFit: 'contain',
                boxShadow: '0 30px 80px rgba(0, 0, 0, 0.4)',
                filter: shouldShowLocked ? 'blur(20px)' : 'none'
              }} autoPlay loop muted playsInline /> : <img key={mediaUrl} src={mediaUrl} alt={`Day ${currentActivity.dayNumber}`} className="w-full h-auto rounded-2xl shadow-2xl" loading="eager" decoding="async" style={{
                maxHeight: 'calc(100dvh - 240px)',
                objectFit: 'contain',
                boxShadow: '0 30px 80px rgba(0, 0, 0, 0.4)',
                filter: shouldShowLocked ? 'blur(20px)' : 'none'
                ...
                if (!img.dataset.retried) {
                  ...
                  img.src = mediaUrl + "?t=" + Date.now();
                    ...
                    {shouldShowLocked && <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-2xl">
                  ...
                  }} className="mt-2 px-6 py-2.5 rounded-full font-semibold text-sm active:scale-95 transition-transform" style={{
                    ...
                    {!shouldShowLocked && <Floating3DEmojis reactions={activeReactionTypes} newReaction={floatingReaction} />}
                  </div>;
          })()}
          </motion.div>
          
          {/* Right arrow indicator with subtle animation */}
          {userGroups.length > 1 && currentUserIndex < userGroups.length - 1 && <motion.button onClick={goNextUser} className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center justify-center" style={{
          width: 40,
          height: 40,
          borderRadius: '50%',
        ...
        <div className="shrink-0 z-40 flex flex-col items-center gap-2" style={{
        paddingBottom: 'env(safe-area-inset-bottom, 0px)'
      }}>
          {(() => {
          const isContentLocked = !isOwnStory && !hasPublicActivity;
          return <div className="w-full flex flex-col items-center" style={{
            ...
            transition: 'opacity 0.2s ease, transform 0.2s ease'
                ...
                <button onClick={() => isOwnStory ? currentReactions.total > 0 && setShowReactsSheet(true) : setShowSendReactionSheet(true)} className="relative overflow-hidden mb-2 active:scale-[0.97] transition-transform" style={{
              ...
              boxShadow: '0 8px 32px rgba(0, 0, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.1)'
                  ...
                  {isOwnStory ? <div className="flex items-center justify-center gap-3 h-full px-5">
                      {currentReactions.total > 0 ? <>
                          <div className="flex -space-x-1">
                            <img src={fireEmoji} alt="fire" className="w-6 h-6 object-contain" style={{
                      filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))'
                    }} />
                            <img src={clapEmoji} alt="clap" className="w-6 h-6 object-contain" style={{
                      filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))'
                    }} />
                          </div>
                    ...
                    </div> : <div className="flex items-center justify-center gap-3 h-full px-5">
                      {currentReactions.total > 0 ? <>
                          <div className="flex -space-x-1">
                            <img src={fireEmoji} alt="fire" className="w-5 h-5 object-contain" style={{
                      filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))'
                    }} />
                            <img src={clapEmoji} alt="clap" className="w-5 h-5 object-contain" style={{
                      filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))'
                    }} />
                          </div>
                          ...
                          <ProfileAvatar src={profile?.avatar_url} name={profile?.display_name || 'You'} size={26} style={{
                    border: '2px solid rgba(255, 255, 255, 0.25)'
                  }} />
        ...
        {/* View Progress button - sticky to bottom with translucent blur */}
          ...
          bottom: 0
            ...
            <button onClick={handleNavigateToProgress} className="w-full flex items-center justify-center gap-2 py-3 cursor-grab active:cursor-grabbing active:bg-white/5 transition-colors" style={{
      ...
      <AnimatePresence>
        {showReactsSheet && <ReactsSoFarSheet activityId={currentActivity?.id || ''} total={currentReactions.total} reactions={currentReactions.reactions} reactorProfiles={currentReactions.reactorProfiles} onClose={() => setShowReactsSheet(false)} onReactionRemoved={loadActivities} />}
      </AnimatePresence>

      {/* Send reaction sheet for all stories */}
      <AnimatePresence>
        {showSendReactionSheet && <SendReactionSheet activityId={currentActivity?.id} currentUserId={user?.id} onReact={handleReact} onClose={() => setShowSendReactionSheet(false)} onViewReactions={() => {
        setShowSendReactionSheet(false);
      ...
      <ReelToProgressTransition isOpen={showProgressOverlay} onClose={handleCloseProgressOverlay} currentActivity={currentActivity ? {
    ...
    } : null} publicFeed={publicFeed.map(p => ({
      id: p.id,
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-white max-w-[320px] rounded-2xl">
          ...
          <AlertDialogFooter className="flex-row gap-3 sm:justify-center">
            <AlertDialogCancel className="flex-1 m-0 bg-zinc-800 border-zinc-700 text-white hover:bg-zinc-700 hover:text-white" disabled={isDeleting}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteActivity} className="flex-1 m-0 bg-red-600 text-white hover:bg-red-700" disabled={isDeleting}>
      ...
      {/* Make Public Sheet */}
      <MakePublicSheet isOpen={showMakePublicSheet} onClose={() => setShowMakePublicSheet(false)} onMakePublic={async () => {
      const latestActivity = myActivities[myActivities.length - 1];
      if (latestActivity) {
